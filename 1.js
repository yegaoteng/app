...
async function musicUrl(source, musicInfo, quality) {
  const songmid = musicInfo.songmid || musicInfo.hash || musicInfo.copyrightId;
  // 这里的 musicInfo.id 在 CeruMusic 中通常已经对应了上述值，但为了保险起见，打印一下
  console.log(`[CeruMusic-LXPlugin] musicUrl called: source=${source}, id=${songmid}, quality=${quality}`);

  const requestPath = `/lxmusicv4/url/${source}/${songmid}/${quality}`;
  const sign = generateSign(requestPath);
  const url = `${API_URL}${requestPath}?sign=${sign}`;

  console.log(`[CeruMusic-LXPlugin] Request URL: ${url}`);
  console.log(`[CeruMusic-LXPlugin] Sign Input: ${requestPath + SCRIPT_MD5 + SECRET_KEY}`);
  console.log(`[CeruMusic-LXPlugin] Generated Sign: ${sign}`);
  ...
  try {
    ...
    console.log(`[CeruMusic-LXPlugin] Response Status: ${result.statusCode}`);
    // console.log(`[CeruMusic-LXPlugin] Response Body:`, result.body);
    ...
    if (typeof data === 'string') {
      try {
        ...
        console.error('[CeruMusic-LXPlugin] JSON Parse Error:', e);
      }
    }

    if (!data || (data.code !== 0 && data.code !== 200)) {
      const msg = data?.msg || data?.message || 'Unknown API error';
      console.error(`[CeruMusic-LXPlugin] API Error: ${msg}, Code: ${data?.code}`);
      throw new Error(msg);
    }

    const finalUrl = data.data || data.url;
    if (!finalUrl) {
      console.error('[CeruMusic-LXPlugin] No URL in response data:', data);
    ...
    console.error(`[${source}] Get music url failed:`, error);
    throw error;
  }
}
...
function generateSign(requestPath) {
  const signInput = requestPath + SCRIPT_MD5 + SECRET_KEY;
...
var sha256 = (function () {
  ...
  Sha256.prototype.update = function (message) {
    ...
    while (index < length) {
      ...
      if (notString) {
        for (i = this.start; index < length && i < 64; ++index) blocks[i >> 2] |= message[index] << SHIFT[i++ & 3];
      } else {
  ...
  Sha256.prototype.hash = function () {
    ...
    for (j = 0; j < 64; j += 4) {
      if (this.first) {
        if (this.is224) {
          ab = 300032; t1 = blocks[0] - 1413257819; h = t1 - 150054599 << 0; d = t1 + 24177077 << 0;
        } else {
          ab = 704751109; t1 = blocks[0] - 210244248; h = t1 - 1521486534 << 0; d = t1 + 143694565 << 0;
        }
        this.first = false;
      } else {
        s0 = ((a >>> 2) | (a << 30)) ^ ((a >>> 13) | (a << 19)) ^ ((a >>> 22) | (a << 10));
        s1 = ((e >>> 6) | (e << 26)) ^ ((e >>> 11) | (e << 21)) ^ ((e >>> 25) | (e << 7));
        ab = a & b; maj = ab ^ (a & c) ^ bc; ch = (e & f) ^ (~e & g);
        t1 = h + s1 + ch + K[j] + blocks[j]; t2 = s0 + maj;
        h = d + t1 << 0; d = t1 + t2 << 0;
      }
      s0 = ((d >>> 2) | (d << 30)) ^ ((d >>> 13) | (d << 19)) ^ ((d >>> 22) | (d << 10));
      s1 = ((h >>> 6) | (h << 26)) ^ ((h >>> 11) | (h << 21)) ^ ((h >>> 25) | (h << 7));
      da = d & a; maj = da ^ (d & b) ^ ab; ch = (h & e) ^ (~h & f);
      t1 = g + s1 + ch + K[j + 1] + blocks[j + 1]; t2 = s0 + maj;
      g = c + t1 << 0; c = t1 + t2 << 0;
      s0 = ((c >>> 2) | (c << 30)) ^ ((c >>> 13) | (c << 19)) ^ ((c >>> 22) | (c << 10));
      s1 = ((g >>> 6) | (g << 26)) ^ ((g >>> 11) | (g << 21)) ^ ((g >>> 25) | (g << 7));
      cd = c & d; maj = cd ^ (c & a) ^ da; ch = (g & h) ^ (~g & e);
      t1 = f + s1 + ch + K[j + 2] + blocks[j + 2]; t2 = s0 + maj;
      f = b + t1 << 0; b = t1 + t2 << 0;
      s0 = ((b >>> 2) | (b << 30)) ^ ((b >>> 13) | (b << 19)) ^ ((b >>> 22) | (b << 10));
      s1 = ((f >>> 6) | (f << 26)) ^ ((f >>> 11) | (f << 21)) ^ ((f >>> 25) | (f << 7));
      bc = b & c; maj = bc ^ (b & d) ^ cd; ch = (f & g) ^ (~f & h);
      t1 = e + s1 + ch + K[j + 3] + blocks[j + 3]; t2 = s0 + maj;
      e = a + t1 << 0; a = t1 + t2 << 0;
    }
